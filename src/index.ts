...
function needsRendering(
  ...
  return (
    renderedRelease.sourceChecksum !== sourceChecksum ||
    renderedRelease.targetChecksum !== targetChecksum ||
    renderedRelease.valuesChecksum !== valuesChecksum
  );
}
...
export async function render(context: KortContext): Promise<void> {
  ...
  for (const env of context.environments) {
    ...
    for (const release of env.helmReleases) {
      ...
      if (needsRendering(release, renderedRelease)) {
        console.log(`    Adding ${release.name} to render plan`);
        plan.push(release);
      } else {
  ...
  for (const release of plan) {
    console.log(`\nRendering ${release.name}...`);
    const success = await renderRelease(release, context.rootDir);
    
    if (success) {
      // Update the rendered state
      const renderedRelease: RenderedRelease = {
        releaseName: release.name,
        sourceChecksum: calculateSourceChecksum(release.chart, release.version),
        targetChecksum: calculateTargetChecksum(release.namespace, release.name),
        valuesChecksum: calculateValuesChecksum(release.valuesObject),
      };

      // Remove existing entry if present
      state.environments = state.environments.filter(
        (r) => r.releaseName !== release.name
      );
      
      // Add updated entry
      state.environments.push(renderedRelease);
      
      console.log(`Successfully rendered ${release.name}`);
    }
  }
